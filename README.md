...
气运修仙是一款基于Python开发的修仙题材挂机游戏，采用客户端-服务端架构。玩家注册账号后即可开始修仙之旅，通过挂机修炼提升境界，体验修仙世界的乐趣。

### 核心特色
- **一账号一数据**: 每个账号对应一个修仙者数据，简化游戏体验
- **挂机修炼系统**: 离线也能持续修炼，提升境界
- **气运系统**: 每日签到获得气运，影响修炼效果和突破成功率
- **装备系统**: 丰富的装备类型，提升修仙者属性
- **背包系统**: 物品管理，支持使用和出售
- **洞府系统**: 个人修炼空间，提升修炼效率
- **灵田系统**: 种植种子，可用作炼丹材料或者售出兑换金币
- **聊天系统**: 与其他玩家交流互动
- **世界Boss**: 挑战强大的Boss获得丰厚奖励
...
- [x] 移除角色等级系统 - 只保留境界系统
- [x] 简化为一账号一数据 - 移除角色概念，每个账号直接对应游戏数据（金币、灵石、修为等）
...
- [x] 实现 `server/database/models.py` - 用户、游戏数据等基础数据模型
...
- [x] 实现 `server/core/systems/combat_system.py` - 战斗逻辑
- [x] 实现副本、怪物、掉落系统
- [x] 实现战斗界面
- [x] 实现世界boss系统 - 完整的boss挑战功能

#### 步骤14：商城交易系统 (4-5天) 🔄 已完成
- [x] 实现 `client/ui/windows/shop_window.py` - 商城界面
- [x] 实现官方商城和玩家交易所
- [x] 实现交易安全机制

#### 步骤15：实时通信 (3-4天) 🔄 部分完成
- [x] 实现 `server/api/v1/websocket.py` - WebSocket服务
- [x] 实现 `client/network/websocket_client.py` - WebSocket客户端
